...
def grab_game_data(browser):
    return browser.execute_script("return document.body.innerHTML")

def grab_game_score(game_data):
    return int(html.fromstring(game_data).xpath('//div[@class="score-container"]/text()')[0])
...
#    game_data = grab_game_data(browser)
#    board_vals = get_board_value(img)
#    score = grab_game_score(game_data)
...
#                post_move_score = int(html.fromstring(browser.execute_script("return document.body.innerHTML")).xpath('//div[@class="score-container"]/text()')[0])
#                print(post_move_score)
#                score_inc = post_move_score > score
#                if score_inc:
#                    same_move_count = 0
#                else:
#                    same_move_count += 1
...
#                this_move_data = pd.DataFrame(np.append(board_vals, [prediction, int(score_inc), score])).transpose()
...
#    training_data.to_csv('v3 Generations Data\\rfc_training_data_' + strftime('%Y_%m_%d_%H_%M') + '.csv', index=False)